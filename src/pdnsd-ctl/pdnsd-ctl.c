...
static const cmd_s top_cmds[]={{"status",CTL_STATS},{"server",CTL_SERVER},{"record",CTL_RECORD},
...
int main(int argc, char *argv[])
{
	...
	print_try_pdnsd_ctl_help:
		...
		case CTL_ADD: {
			...
			short tp,flags;
			if (argc<2 ||
			    argc<(acnt=(((tp=match_cmd(argv[1],rectype_cmds))==T_MX)?5:4)) ||
			    argc>acnt+2)
			{
				goto wrong_args;
			}
			...
			case T_MX:
				{
					short pref;
					if (sscanf(argv[4], "%hd", &pref)!=1) {
						fprintf(stderr,"Bad preference number for 'add mx' command: %s\n",argv[4]);
						exit(2);
					}
					send_short(pf,pref);
					send_string(pf,argv[2]);
